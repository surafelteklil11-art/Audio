...
object MediaItemMenuInstaller {
    ...
    private const val VIDEO_MORE_TAG = "video_more_button"
    ...
    private fun attachAll(view: View, activity: Activity) {
        if (view.id == R.id.songMore && view.getTag(R.id.media_menu_installed_tag) != true) {
            view.setTag(R.id.media_menu_installed_tag, true)
            view.setOnClickListener { showMenu(activity, view, false) }
        }
        if (view is ViewGroup) {
            val videoTitle = view.findViewById<View>(R.id.videoTitle)
            if (videoTitle != null && view.findViewWithTag<View>(VIDEO_MORE_TAG) == null) {
                val more = TextView(activity).apply {
                    tag = VIDEO_MORE_TAG
                    text = "⋮"
                    textSize = 21f
                    gravity = Gravity.CENTER
                    setTextColor(Color.rgb(137, 147, 173))
                    contentDescription = "More options"
                    isClickable = true
                    isFocusable = true
                    setOnClickListener { showMenu(activity, this, true) }
                }
                view.addView(more, ViewGroup.LayoutParams(dp(activity, 34), dp(activity, 64)))
            }
            for (i in 0 until view.childCount) attachAll(view.getChildAt(i), activity)
        }
    }

    private fun showMenu(activity: Activity, moreView: View, isVideo: Boolean) {
        val item = resolveItem(moreView) ?: return
        if (isVideo && item !is VideoEntry) return
        if (!isVideo && item !is MediaItem) return
        ...
        val actions = if (isVideo) arrayOf("Play next", "Rename", "Share", "Details", "Remove from device") else arrayOf("Play next", "Add to queue", "Rename", "Share", "Details", "Remove from device")
        android.app.AlertDialog.Builder(activity).setTitle(title).setItems(actions) { _, which ->
            ...
            when {
                ...
                (!isVideo && which == 5) || (isVideo && which == 4) -> removeFromDevice(activity, uri, title)
            }
        }.show()
    }
    ...
    private fun playNext(activity: Activity, item: MediaItem) = withController(activity) { c ->
        if (c.mediaItemCount == 0) {
            ...
            val insertAt = if (c.currentMediaItemIndex >= 0) c.currentMediaItemIndex + 1 else c.mediaItemCount
            c.addMediaItem(insertAt.coerceIn(0, c.mediaItemCount), item)
        }
    }
    ...
    private fun removeFromDevice(activity: Activity, uri: Uri, title: String) {
    ...
    private fun dp(activity: Activity, value: Int): Int = (value * activity.resources.displayMetrics.density).toInt().coerceAtLeast(1)
    ...
    private fun resolveItem(moreView: View): Any? {
        ...
        return runCatching {
            ...
            (field.get(adapter) as? List<*>)?.getOrNull(pos)
        }.getOrNull()
    }
}
