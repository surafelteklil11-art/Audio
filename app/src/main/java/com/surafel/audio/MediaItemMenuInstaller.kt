...
/** Binds normal Audio/Video overflow buttons after RecyclerView rows are created. */
object MediaItemMenuInstaller {
    ...
    fun install(activity: Activity) {
        ...
        AudioLibraryTabsInstaller.install(activity)
    }
    ...
    private fun findMediaUri(activity: Activity, title: String, video: Boolean): Uri? {
        ...
        return activity.contentResolver.query(collection, projection, selection, arrayOf(title, title), null)?.use { cursor ->
    ...
    private fun share(activity: Activity, uri: Uri, title: String, mime: String) {
        val send = Intent(Intent.ACTION_SEND).apply { type = mime; putExtra(Intent.EXTRA_STREAM, uri); putExtra(Intent.EXTRA_TITLE, title); addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION) }
        activity.startActivity(Intent.createChooser(send, "Share $title"))
    }

    private fun details(activity: Activity, uri: Uri, title: String) {
        val message = activity.contentResolver.query(uri, arrayOf(MediaStore.MediaColumns.DISPLAY_NAME, MediaStore.MediaColumns.SIZE, MediaStore.MediaColumns.MIME_TYPE), null, null, null)?.use { cursor ->
            if (cursor.moveToFirst()) "Name: ${cursor.getString(0) ?: title}\nSize: ${formatSize(cursor.getLong(1))}\nType: ${cursor.getString(2) ?: "Unknown"}" else title
        } ?: title
