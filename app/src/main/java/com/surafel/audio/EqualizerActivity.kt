...
class EqualizerActivity : AudioToolPageActivity() {
    ...
    private class SciFrameDrawable(
        ...
        override fun draw(canvas: Canvas) {
            ...
            val innerCut = max(3f, cut - activity.dp(4))
            path.reset()
            path.moveTo(b.left + innerCut, b.top + activity.dp(5))
            path.lineTo(b.right - innerCut, b.top + activity.dp(5))
            path.lineTo(b.right - activity.dp(4), b.top + innerCut)
            path.lineTo(b.right - activity.dp(4), b.bottom - innerCut)
            path.lineTo(b.right - innerCut, b.bottom - activity.dp(5))
            path.lineTo(b.left + innerCut, b.bottom - activity.dp(5))
            path.lineTo(b.left + activity.dp(4), b.bottom - innerCut)
            path.lineTo(b.left + activity.dp(4), b.top + innerCut)
            path.close()
            paint.color = magenta
            paint.strokeWidth = activity.dp(1).toFloat()
            canvas.drawPath(path, paint)
