...
class MainActivity : AppCompatActivity() {
    ...
    private val permissionLauncher = registerForActivityResult(ActivityResultContracts.RequestPermission()) { granted -> if (granted) loadTab() }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<ImageButton>(R.id.play).setOnClickListener { if (player.isPlaying) player.pause() else if (player.mediaItemCount > 0) player.play() }
        findViewById<TextView>(R.id.playAll).setOnClickListener { if (items.isNotEmpty()) playFrom(0) }
        findViewById<TextView>(R.id.shuffleAll).setOnClickListener { if (items.isNotEmpty()) { player.setMediaItems(items.shuffled(), 0, 0L); player.prepare(); player.play() } }
    ...
    private fun selectTab(tab: Tab) { currentTab = tab; updateTabStyle(); if (::player.isInitialized && hasAudioPermission()) loadTab() }
    ...
    private fun updateTabStyle() {
        ...
        ids.forEachIndexed { index, id -> findViewById<TextView>(id).apply { setTextColor(if (index == selected) Color.WHITE else Color.rgb(101,113,139)); textSize = if (index == selected) 25f else 22f; setTypeface(typeface, if (index == selected) android.graphics.Typeface.BOLD else android.graphics.Typeface.NORMAL) } }
    }

    private fun playFrom(position: Int) {
        if (position !in items.indices) return
        val item = items[position]
        if (item.localConfiguration?.uri == android.net.Uri.EMPTY) return
        player.setMediaItem(item, 0L); player.prepare(); player.play()
    }

    private fun requestAudioPermissionIfNeeded() {
        val permission = if (Build.VERSION.SDK_INT >= 33) Manifest.permission.READ_MEDIA_AUDIO else Manifest.permission.READ_EXTERNAL_STORAGE
        if (ContextCompat.checkSelfPermission(this, permission) == PackageManager.PERMISSION_GRANTED) loadTab() else permissionLauncher.launch(permission)
    }
    ...
    private fun loadPlaylists() {
        ...
        contentResolver.query(playlists,arrayOf(MediaStore.Audio.Playlists._ID,MediaStore.Audio.Playlists.NAME),null,null,"${MediaStore.Audio.Playlists.NAME} COLLATE NOCASE ASC")?.use { c ->
            ...
            while(c.moveToNext()) {
                val playlistId=c.getLong(pid); val members=MediaStore.Audio.Playlists.Members.getContentUri("external",playlistId)
                var firstUri=android.net.Uri.EMPTY; var count=0
                contentResolver.query(members,arrayOf(MediaStore.Audio.Playlists.Members.AUDIO_ID),null,null,null)?.use { m ->
                    val audioId=m.getColumnIndexOrThrow(MediaStore.Audio.Playlists.Members.AUDIO_ID)
                    if(m.moveToFirst()){ firstUri=android.content.ContentUris.withAppendedId(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI,m.getLong(audioId)); count=m.count }
                }
                found += mediaItem(firstUri,c.getString(name),"Playlist • $count songs")
            }
        }
        replaceItems(found)
    }
