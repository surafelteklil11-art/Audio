...
class AudioLibraryPageActivity : Activity() {
    ...
    private fun playlists() {
        ...
        contentResolver.query(MediaStore.Audio.Playlists.EXTERNAL_CONTENT_URI,
            ...
            "${MediaStore.Audio.Playlists.NAME} COLLATE NOCASE ASC")?.use { c ->
    ...
    private fun folders() {
        ...
        contentResolver.query(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI,
            arrayOf(MediaStore.Audio.Media.DATA), "${MediaStore.Audio.Media.IS_MUSIC} != 0", null,
            "${MediaStore.Audio.Media.DATA} COLLATE NOCASE ASC")?.use { c ->
            val data = c.getColumnIndexOrThrow(MediaStore.Audio.Media.DATA)
            while (c.moveToNext()) {
                val p = c.getString(data) ?: continue
                folders += File(p).parent ?: "/"
            }
        }
        folders.sortedWith(String.CASE_INSENSITIVE_ORDER).forEach { folder ->
            row(folder.substringAfterLast('/').ifBlank { folder }) { songs(queryFolder(folder)) }
        }
    ...
    private fun artists() {
        ...
        contentResolver.query(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI,
            ...
            "${MediaStore.Audio.Media.ARTIST} COLLATE NOCASE ASC")?.use { c ->
            while (c.moveToNext()) values += c.getString(0) ?: "Unknown artist"
        }
        values.filter { it.isNotBlank() }.sortedWith(String.CASE_INSENSITIVE_ORDER).forEach { value ->
            row(value) { songs(querySongs(MediaStore.Audio.Media.ARTIST, value)) }
        }
    ...
    private fun albums() {
        val values = linkedSetOf<String>()
        contentResolver.query(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI,
            arrayOf(MediaStore.Audio.Media.ALBUM), "${MediaStore.Audio.Media.IS_MUSIC} != 0", null,
            "${MediaStore.Audio.Media.ALBUM} COLLATE NOCASE ASC")?.use { c ->
            while (c.moveToNext()) values += c.getString(0) ?: "Unknown album"
        }
        values.filter { it.isNotBlank() }.sortedWith(String.CASE_INSENSITIVE_ORDER).forEach { value ->
            row(value) { songs(querySongs(MediaStore.Audio.Media.ALBUM, value)) }
        }
        if (values.isEmpty()) empty()
    }
    ...
    private fun row(text: String, action: () -> Unit) {
        val v = TextView(this).apply {
            ...
            setPadding(dp(18), dp(12), dp(18), dp(12)); minHeight = dp(64)
    ...
    private fun empty() {
        list.removeAllViews()
        // Intentionally blank: an empty library page should remain empty, not show a dialog.
    }
    ...
    private fun querySongs(column: String?, value: String?): List<MediaItem> {
        ...
        contentResolver.query(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, p, selection, args,
    ...
    private fun queryFolder(folder: String): List<MediaItem> {
        ...
        contentResolver.query(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, p, "${MediaStore.Audio.Media.IS_MUSIC} != 0", null,
            "${MediaStore.Audio.Media.TITLE} COLLATE NOCASE ASC")?.use { c ->
            val id = c.getColumnIndexOrThrow(MediaStore.Audio.Media._ID); val t = c.getColumnIndexOrThrow(MediaStore.Audio.Media.TITLE)
            val a = c.getColumnIndexOrThrow(MediaStore.Audio.Media.ARTIST); val d = c.getColumnIndexOrThrow(MediaStore.Audio.Media.DATA)
    ...
    private fun queryPlaylist(id: Long): List<MediaItem> {
        ...
        contentResolver.query(uri, arrayOf(MediaStore.Audio.Playlists.Members.AUDIO_ID, MediaStore.Audio.Media.TITLE, MediaStore.Audio.Media.ARTIST), null, null,
            "${MediaStore.Audio.Playlists.Members.PLAY_ORDER} ASC")?.use { c ->
    ...
    private fun item(id: Long, title: String?, artist: String?) = MediaItem.Builder()
        .setUri(ContentUris.withAppendedId(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, id))
        .setMediaMetadata(androidx.media3.common.MediaMetadata.Builder().setTitle(title ?: "Unknown").setArtist(artist ?: "Unknown artist").build()).build()
    ...
    companion object {
        ...
        const val SECTION_SONGS = "songs"; const val SECTION_PLAYLISTS = "playlists"; const val SECTION_FOLDERS = "folders"; const val SECTION_ARTISTS = "artists"; const val SECTION_ALBUMS = "albums"
