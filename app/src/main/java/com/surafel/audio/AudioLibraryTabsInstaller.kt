...
object AudioLibraryTabsInstaller {
    ...
    private fun showPlaylists(activity: MainActivity) {
        ...
        if (rows.isEmpty()) return info(activity, "Playlists", "No playlists found on this device.")
    ...
    private fun showFolders(activity: MainActivity) {
        ...
        activity.contentResolver.query(base, arrayOf(MediaStore.Audio.Media.DATA), "${MediaStore.Audio.Media.IS_MUSIC} != 0", null, "${MediaStore.Audio.Media.DATA} COLLATE NOCASE ASC")?.use { c ->
            ...
            while (c.moveToNext()) folders += File(c.getString(data) ?: continue).parent ?: "/"
        }
        val values = folders.toList().sorted(String.CASE_INSENSITIVE_ORDER)
        if (values.isEmpty()) return info(activity, "Folders", "No audio folders found.")
    ...
    private fun showFolderSongs(activity: MainActivity, folder: String) {
        ...
        activity.contentResolver.query(base, projection, "${MediaStore.Audio.Media.IS_MUSIC} != 0", null, "${MediaStore.Audio.Media.TITLE} COLLATE NOCASE ASC")?.use { c ->
            ...
            while (c.moveToNext()) if (File(c.getString(data) ?: "").parent == folder) songs += buildItem(c.getLong(id), c.getString(title), c.getString(artist))
        }
    ...
    private fun showArtists(activity: MainActivity) {
        ...
        activity.contentResolver.query(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, arrayOf(MediaStore.Audio.Media.ARTIST), "${MediaStore.Audio.Media.IS_MUSIC} != 0", null, "${MediaStore.Audio.Media.ARTIST} COLLATE NOCASE ASC")?.use { c -> while (c.moveToNext()) artists += c.getString(0) ?: "Unknown artist" }
        val values = artists.filter { it.isNotBlank() }.sorted(String.CASE_INSENSITIVE_ORDER)
        if (values.isEmpty()) return info(activity, "Artists", "No artists found.")
    ...
    private fun showAlbums(activity: MainActivity) {
        ...
        activity.contentResolver.query(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, arrayOf(MediaStore.Audio.Media.ALBUM), "${MediaStore.Audio.Media.IS_MUSIC} != 0", null, "${MediaStore.Audio.Media.ALBUM} COLLATE NOCASE ASC")?.use { c -> while (c.moveToNext()) albums += c.getString(0) ?: "Unknown album" }
        val values = albums.filter { it.isNotBlank() }.sorted(String.CASE_INSENSITIVE_ORDER)
        if (values.isEmpty()) return info(activity, "Albums", "No albums found.")
    ...
    private fun showFilteredSongs(activity: MainActivity, title: String, column: String, value: String) {
        val songs = mutableListOf<MediaItem>(); val base = MediaStore.Audio.Media.EXTERNAL_CONTENT_URI
    ...
    private fun showPlayableList(activity: MainActivity, title: String, songs: List<MediaItem>) {
        if (songs.isEmpty()) return info(activity, title, "No audio items found.")
    ...
    private fun info(activity: Activity, title: String, message: String) = AlertDialog.Builder(activity).setTitle(title).setMessage(message).setPositiveButton("OK", null).show()
}
