...
- Reading uses continuous vertical scrolling: pages appear one below another at the document width. Only attached pages are rendered on a worker, with bounded bitmap sizes and cancelled recycled-page requests. Swipe up/down, pinch zoom, pan sideways while zoomed, double-tap to save a screenshot, or use page navigation in View mode / Manage. The last visible page is remembered across reopening and rotation. Page night mode is separate from library appearance.
